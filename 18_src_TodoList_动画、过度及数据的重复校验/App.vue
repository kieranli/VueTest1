 ...
 * @LastEditTime: 2022-08-23 21:14:41
...
export default {
  ...
  methods:{
    // 添加todo
    addTodo(value){
      // reduce计算出重名的任务数量，大于0则return
      const todoNum = this.todos.reduce((pre,todo)=>{
          return pre + (todo.title === value.title ? 1:0)
      },0)
      if(todoNum>0) return alert('该任务已存在，不能重复添加！')
